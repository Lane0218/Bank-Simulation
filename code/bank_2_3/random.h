...
public:
    ...
    // generate random integer a <= value <= b-1
    unsigned short Random(unsigned long a, unsigned long b);
...
unsigned short RandomNumber::Random(unsigned long a, unsigned long b)
{
    randSeed = multiplier * randSeed + adder;
    return (unsigned short)((randSeed >> 16) % (b - a) + a);
}
